...
from mvp_platform import config, criteria_stage, qa_stage, store, production_stage
...
def index():
    ...
    if selected:
        ...
        # Inject grouped_artifacts into pipeline stages so the UI template can render them
        for stage in ["intake", "criteria", "production", "qa"]:
            if stage not in pipeline:
                pipeline[stage] = {}
            pipeline[stage]["artifacts"] = grouped_artifacts.get(stage, [])
...
@app.post("/projects/<project_id>/run-production")
...
@app.get("/projects/<project_id>/open-folder/<stage>")
def open_folder(project_id: str, stage: str):
    import threading
    target = (store.project_dir(project_id) / stage).resolve()
...
if __name__ == "__main__":
    # Only open browser once (don't open on reload)
